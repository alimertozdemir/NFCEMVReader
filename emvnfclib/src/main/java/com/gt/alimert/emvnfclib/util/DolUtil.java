...
import com.gt.alimert.emvnfclib.model.TlvObject;
...
public final class DolUtil {
    ...
    @Nullable
    public static byte[] generateDol(@Nullable byte[] dol, byte[] amount) {
        // Returning result
        byte[] result = null;
        // - Returning result
        ...
        if (dol != null) {
            for (int i = 0; i < dol.length; i++) {
                ...
                TlvObject tlvObj = new TlvObject(tlvTag, dol[goNext]);
        ...
        try {
            ...
            if (dol != null) {
                for (TlvObject tlvObject : tlvObjectArrayList) {
                    byte[] generateDolResult = new byte[tlvObject.getTlvTagLength()];
                    ...
                    // UN (Unpredictable Number); 9F37, 1 or 4 Byte(s)
                    else if (Arrays.equals(tlvObject.getTlvTag(), TlvTagConstant.UNPREDICTABLE_NUMBER_TLV_TAG)) {
                        ...
                        if (unSecureRandom != null) {
                            try {
                                unSecureRandom.nextBytes(generateDolResult);
                            } catch (Exception e) {
                                Log.e(TAG, e.toString());
                            }
                        }
                    ...
                    if (resultValue != null) {
                        try {
                            System.arraycopy(resultValue, 0, generateDolResult, 0, Math.min(resultValue.length, generateDolResult.length));
                        } catch (Exception e) {
                            Log.e(TAG, e.toString());
                        }
                    }

                    byteArrayOutputStream.write(generateDolResult); // Data
                }
            }
